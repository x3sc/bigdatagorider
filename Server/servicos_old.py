...
from .utils import get_db_connection as get_db_conn
...
def create_proposta(servico_id: int, proposta_data: PropostaCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("SELECT ID_Servico FROM Servicos WHERE ID_Servico = ? AND Status = 'Aberto'", servico_id)
...
def get_propostas_for_servico(servico_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("SELECT ID_Cliente FROM Servicos WHERE ID_Servico = ?", servico_id)
        servico_owner = cursor.fetchone()
        if not servico_owner or servico_owner[0] != current_user['id']:
...
def aceitar_proposta(proposta_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        id_servico, id_prestador, id_veiculo, valor_proposto, id_cliente = proposta
        ...
        query_update_servico = """
            UPDATE Servicos
            SET ID_Prestador_Aceito = ?, ID_Veiculo_Alocado = ?, ValorFinalAcordado = ?, Status = 'Em Andamento'
            WHERE ID_Servico = ? AND Status = 'Aberto'
        """
        ...
        cursor.execute("UPDATE PropostasServico SET Status = 'Aceita' WHERE ID_Proposta = ?", proposta_id)

        # 3. Recusa as outras propostas para o mesmo serviço
        cursor.execute("UPDATE PropostasServico SET Status = 'Recusada' WHERE ID_Servico = ? AND ID_Proposta != ?", id_servico, proposta_id)
    ...
    try:
        if db_status == 'Aberto':
            ...
            query = """
                ...
                FROM Servicos s
                JOIN Usuarios u ON s.ID_Cliente = u.ID_Usuario
                WHERE s.ID_Prestador_Aceito = ? AND s.Status = ?
            """
        ...
        for row in rows:
            servico_item = {
                ...
                "data_servico": str(row.DataSolicitacao if hasattr(row, 'DataSolicitacao') else row.data_criacao),
                "status": db_status
            }
            if db_status == 'Aberto':
                servico_item.update({
...
def get_cliente_servicos_por_status(status: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        query = f"""
            ...
            WHERE s.ID_Cliente = ? AND s.Status IN ({placeholders})
...
def finalizar_servico_prestador(servico_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        cursor.execute(
            "SELECT ID_Servico FROM Servicos WHERE ID_Servico = ? AND ID_Prestador_Aceito = ? AND Status = 'Em Andamento'",
        ...
        cursor.execute("UPDATE Servicos SET Status = 'Aguardando Confirmação' WHERE ID_Servico = ?", servico_id)
        conn.commit()
...
def confirmar_servico_cliente(servico_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        cursor.execute(
            "SELECT ID_Servico FROM Servicos WHERE ID_Servico = ? AND ID_Cliente = ? AND Status = 'Aguardando Confirmação'",
        ...
        cursor.execute("UPDATE Servicos SET Status = 'Concluido', DataConclusao = ? WHERE ID_Servico = ?", datetime.now(), servico_id)
        conn.commit()
